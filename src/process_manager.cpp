...
ProcessManager::ProcessManager()
    : prev_uptime(0), first_global_cpu_run(true), first_process_run(true) {
...
long ProcessManager::get_hertz() { return hertz; }
...
MemHardwareInfo ProcessManager::get_memory_hardware_info() {
  ...
  while (fgets(buffer, sizeof(buffer), fp) != NULL) {
    ...
    if (line == "Memory Device") {
      ...
      } else if (line.find("Type:") != std::string::npos && device_has_size &&
                 line.find("Type Detail") == std::string::npos) {
        size_t pos = line.find(":");
        if (pos != std::string::npos) {
          std::string val = line.substr(pos + 1);
          val.erase(0, val.find_first_not_of(" \t"));
      ...
      } else if (line.find("Speed:") != std::string::npos && device_has_size &&
                 line.find("Configured") == std::string::npos) {
        size_t pos = line.find(":");
        if (pos != std::string::npos) {
          std::string val = line.substr(pos + 1);
          val.erase(0, val.find_first_not_of(" \t"));
            ...
            if (rp != std::string::npos) {
              ...
              if (!first_process_run && prev_process_cpu.count(pid) > 0) {
                ...
                if (uptime_diff > 0) {
                  ...
                  unsigned long long total_time_diff =
                      (utime + stime) - (prev_utime + prev_stime);
                  info.cpu_usage = (100.0 * ((double)total_time_diff / hertz) /
                                    uptime_diff) /
                                   get_cpu_threads_count();
                }
              }
            }
          }
...
bool ProcessManager::kill_process(int pid) { return kill(pid, SIGTERM) == 0; }
