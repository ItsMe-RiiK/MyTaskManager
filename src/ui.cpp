...
UIManager::UIManager()
    : window(nullptr), notebook(nullptr), current_perf_tab(0) {
...
void UIManager::setup_process_tab(GtkWidget *parent) {
  ...
  process_filter =
      gtk_tree_model_filter_new(GTK_TREE_MODEL(process_store), NULL);
  gtk_tree_model_filter_set_visible_func(GTK_TREE_MODEL_FILTER(process_filter),
                                         on_search_filter_visible, this, NULL);
  ...
  const char *headers[] = {"PID",     "Apps Name",  "User", "Status",
                           "CPU (%)", "Memory (%)", "BG",   "FG"};
  ...
  for (int i : col_indices) {
    ...
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
        headers[i], renderer, "text", i, "cell-background", COL_BG_COLOR,
  ...
  GtkWidget *menu_item_open =
      gtk_menu_item_new_with_label("Open file location");
  ...
  g_signal_connect(menu_item_end, "activate",
                   G_CALLBACK(on_context_menu_end_task), this);
  g_signal_connect(menu_item_open, "activate",
                   G_CALLBACK(on_context_menu_open_file_location), this);

  g_signal_connect(process_treeview, "button-press-event",
                   G_CALLBACK(on_process_button_press), this);
}
...
void UIManager::setup_performance_tab(GtkWidget *parent) {
  ...
  g_signal_connect(sidebar, "row-activated",
                   G_CALLBACK(on_perf_sidebar_row_activated), this);
  ...
  g_signal_connect(G_OBJECT(perf_cpu_drawing_area), "draw",
                   G_CALLBACK(on_draw_cpu_graph), this);
  ...
  g_signal_connect(G_OBJECT(perf_mem_drawing_area), "draw",
                   G_CALLBACK(on_draw_mem_graph), this);

  mem_details_label = gtk_label_new(
      "Speed: -\nSlots used: -\nForm factor: -\nHardware reserved: -");
...
void UIManager::setup_service_tab(GtkWidget *parent) {
  ...
  service_store =
      gtk_list_store_new(NUM_SVC_COLS, G_TYPE_STRING, G_TYPE_STRING,
                         G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
  service_filter =
      gtk_tree_model_filter_new(GTK_TREE_MODEL(service_store), NULL);
  gtk_tree_model_filter_set_visible_func(GTK_TREE_MODEL_FILTER(service_filter),
                                         on_service_search_filter_visible, this,
                                         NULL);
  ...
  const char *headers[] = {"Service Name", "Load", "Active", "Sub",
                           "Description"};
  ...
  g_signal_connect(btn_start, "clicked", G_CALLBACK(on_start_service_clicked),
                   this);
  g_signal_connect(btn_stop, "clicked", G_CALLBACK(on_stop_service_clicked),
                   this);
  g_signal_connect(btn_restart, "clicked",
...
void UIManager::refresh_performance() {
  ...
  snprintf(
      mem_details_text, sizeof(mem_details_text),
      "In Use: %.1f GB\nAvailable: %.1f GB\nCached: %.1f GB\nSwap: %.1f / %.1f "
      "GB\n\nSpeed: %s\nSlots used: %d of %d\nForm factor: %s\nType: %s%s",
      used_gb, avail_gb, cached_gb, swap_used_gb, swap_tot_gb,
      mem_hw_info.speed.c_str(), mem_hw_info.slots_used,
      mem_hw_info.slots_total, mem_hw_info.form_factor.c_str(),
      mem_hw_info.type.c_str(),
...
void UIManager::refresh_services() {
  ...
  for (const auto &s : services) {
    ...
    gtk_list_store_set(service_store, &iter, COL_SVC_NAME, s.name.c_str(),
                       COL_SVC_LOAD, s.load_state.c_str(), COL_SVC_ACTIVE,
                       s.active_state.c_str(), COL_SVC_SUB, s.sub_state.c_str(),
                       COL_SVC_DESC, s.description.c_str(), -1);
  }
}
...
gboolean UIManager::on_draw_cpu_graph(GtkWidget *widget, cairo_t *cr,
                                      gpointer user_data) {
  UIManager *ui = static_cast<UIManager *>(user_data);
  ...
  for (int i = 1; i < ui->max_history; ++i) {
    cairo_line_to(cr, i * step_x,
                  height - (ui->cpu_history[i] / 100.0) * height);
  }
...
gboolean UIManager::on_draw_mem_graph(GtkWidget *widget, cairo_t *cr,
                                      gpointer user_data) {
  UIManager *ui = static_cast<UIManager *>(user_data);
  guint width = gtk_widget_get_allocated_width(widget);
  guint height = gtk_widget_get_allocated_height(widget);
  ...
  for (int i = 1; i < ui->max_history; ++i) {
    cairo_line_to(cr, i * step_x,
                  height - (ui->mem_history[i] / 100.0) * height);
  }
  cairo_stroke_preserve(cr);
  cairo_line_to(cr, width, height);
  cairo_line_to(cr, 0, height);
...
void UIManager::on_kill_process_clicked(GtkWidget *widget, gpointer user_data) {
  ...
  GtkTreeSelection *selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(ui->process_treeview));
  GtkTreeModel *model;
  GtkTreeIter iter;
...
gboolean UIManager::on_process_button_press(GtkWidget *widget,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 3) {
    ...
    if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), (gint)event->x,
      ...
      gtk_menu_popup_at_pointer(GTK_MENU(ui->process_context_menu),
                                (GdkEvent *)event);
...
void UIManager::on_search_changed(GtkSearchEntry *entry, gpointer data) {
  ...
  std::transform(ui->search_query.begin(), ui->search_query.end(),
                 ui->search_query.begin(), ::tolower);
...
gboolean UIManager::on_window_key_press(GtkWidget *widget, GdkEventKey *event,
                                        gpointer user_data) {
  (void)widget;
  UIManager *ui = static_cast<UIManager *>(user_data);

  if (ui->search_entry) {
    return gtk_search_entry_handle_event(GTK_SEARCH_ENTRY(ui->search_entry),
                                         (GdkEvent *)event);
  }
  return FALSE;
}

gboolean UIManager::on_search_filter_visible(GtkTreeModel *model,
...
void UIManager::on_context_menu_end_task(GtkMenuItem *item,
                                         gpointer user_data) {
  (void)item;
  UIManager *ui = static_cast<UIManager *>(user_data);
  GtkTreeSelection *selection =
...
void UIManager::on_context_menu_open_file_location(GtkMenuItem *item,
                                                   gpointer user_data) {
  (void)item;
  UIManager *ui = static_cast<UIManager *>(user_data);
  GtkTreeSelection *selection =
...
void UIManager::on_perf_sidebar_row_activated(GtkListBox *box,
                                              GtkListBoxRow *row,
                                              gpointer user_data) {
  (void)box;
  UIManager *ui = static_cast<UIManager *>(user_data);
...
void UIManager::on_start_service_clicked(GtkWidget *widget,
                                         gpointer user_data) {
  (void)widget;
  UIManager *ui = static_cast<UIManager *>(user_data);
  GtkTreeSelection *selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(ui->service_treeview));
  GtkTreeModel *model;
  GtkTreeIter iter;
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    gchar *name;
...
void UIManager::on_restart_service_clicked(GtkWidget *widget,
                                           gpointer user_data) {
  (void)widget;
  UIManager *ui = static_cast<UIManager *>(user_data);
  GtkTreeSelection *selection =
